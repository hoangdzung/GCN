...
def main(args):
    ...
    if args.net_type == 'gcn':
        model = GCNNet(attr_matrix.shape[1], args.embedding_size, args.slope, args.temp)
    elif args.net_type == 'gat':
        model = GATNet(attr_matrix.shape[1], args.embedding_size, args.slope, args.temp)
    elif args.net_type == 'sage':
        model = SAGENet(attr_matrix.shape[1], args.embedding_size, args.slope, args.temp)
    ...
    for i in tqdm(range(args.n_epochs)):
        edge_idxs = list(range(n_edges))
        random.shuffle(edge_idxs)
        while(len(edge_idxs)>0):
            edge_idx = edge_idxs[:min(args.batch_size, len(edge_idxs))]
            edge_idxs = edge_idxs[min(args.batch_size, len(edge_idxs)):]

            edge_index_batch = edge_index[:,edge_idx]
            model.train()
            optimizer.zero_grad()
            embedding = model(attr_matrix, edge_index_batch)
